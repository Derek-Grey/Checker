...
class StrategyPlotter:
    ...
    def plot_net_value(self, df: pd.DataFrame, strategy_name: str):
        ...
        if 'time' in df.columns:
            ...
            # 筛选出每个日期组中时间为15:00:00的记录
            # 如果没有精确的15:00:00，则取最接近的时间点
            filtered_df = []
            for date, group in df.groupby('date_only'):
                # 尝试找到精确的15:00:00记录
                target_time = pd.to_datetime(f"{date} 15:00:00")
                exact_match = group[group['datetime'] == target_time]
                
                if not exact_match.empty:
                    # 如果找到精确匹配，使用它
                    filtered_df.append(exact_match)
                else:
                    # 否则找最接近的时间点
                    group['time_diff'] = abs(group['datetime'] - target_time)
                    closest_record = group.loc[group['time_diff'].idxmin()].to_frame().T
                    filtered_df.append(closest_record)
            
            # 合并结果
            if filtered_df:
                df = pd.concat(filtered_df, ignore_index=True)
                print(f"已将分钟频数据筛选为每日15:00:00的数据，共 {len(df)} 行")
            else:
                print("警告：筛选后没有数据")
